...
export async function register() {
  ...
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    try {
      ...
      console.warn('[instrumentation] Database initialization failed (may be on serverless):', err instanceof Error ? err.message : err);
      // Continue anyway — API handlers will fail gracefully if needed
    }
  }
}
